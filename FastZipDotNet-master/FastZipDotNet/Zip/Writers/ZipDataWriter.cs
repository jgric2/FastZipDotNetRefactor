...
namespace FastZipDotNet.Zip.Writers
{
    public class ZipDataWriter
    {
        ...
        // Default threshold for in-memory vs streaming path (4 MB)
        private const long DefaultBufferThreshold = 4L * 1024 * 1024;
            ...
            uint? externalAttr = null)
        {
            ...
            if (doAes)
            {
                ...
                if (payload.Length > 0)
                {
                    ...
                    {
                        // Already reported uncompressed progress in the compression stage (for streaming path),
                        // but AddBuffer is non-streaming. Keep existing behavior (estimate by ratio) here.
            ...
            else if (doZipCrypto)
            {
                ...
                if (payload.Length > 0)
                {
                    ...
                    {
                        // Keep existing behavior for AddBuffer
            ...
            try
            {
                ...
                else
                {
                    ...
                    using (var temp = new FileStream(tempFilePath, FileMode.Create, FileAccess.ReadWrite, FileShare.Read, Consts.ChunkSize, FileOptions.SequentialScan))
                    {
                        if (zfe.Method == Compression.Deflate)
                        {
                            // Report uncompressed progress during compression stage (smooth UI).
                            LibDeflateWrapper.DeflateStreamToStream(
                                ...
                                onReadProgress: n => onUncompressedProgress?.Invoke(n),
                        ...
                        else if (zfe.Method == Compression.Zstd)
                        {
                            ...
                            {
                                WaitWhilePaused();
                                // Report uncompressed progress while compressing to temp
                                onUncompressedProgress?.Invoke(r);
                                zstd.Write(buf, 0, r);
                            }
                            zstd.Flush();
                        }
                ...
                if (doAes)
                {
                    ...
                    if (zfe.Method == Compression.Store)
                    {
                        // Store: report both (no compression stage progress)
                        WriteStreamAt(
                            dataSource,
                    ...
                    {
                        // Compressed case: we've already reported uncompressed progress in compression stage.
                        // Do NOT report uncompressed here to avoid double counting. Only compressed write progress.
                        WriteStreamAt(
                            dataSource,
                            start + localHeader.Length + saltLen + 2,
                            onUncompressedProgress: null, // NO-OP for uncompressed here
                            onCompressedWrite: onCompressedWrite,
                            transform: transform);
                    }
                ...
                else if (doZipCrypto)
                {
                    ...
                    {
                        // Compressed: uncompressed progress already reported in compression stage
                        WriteStreamAt(
                            dataSource,
                            start + localHeader.Length + encHeader.Length,
                            onUncompressedProgress: null,
                            onCompressedWrite: onCompressedWrite,
                            transform: transform);
                    }
                ...
                else
                {
                    ...
                    if (zfe.Method == Compression.Store)
                    {
                        // Store: no compression stage -> report both here
                        WriteStreamAt(
                            dataSource,
                    ...
                    {
                        // Deflate/Zstd: already reported uncompressed during compression stage
                        WriteStreamAt(
                            dataSource,
                            start + localHeader.Length,
                            onUncompressedProgress: null,
        ...
        public void AddEmptyFolder(string folderNameInZip, string fileComment = "")
        {
            try
            {
                ...
                var zfe = new ZipFileEntry
                {
                    FilenameInZip = IOHelpers.NormalizedFilename(folderNameInZip), // retains trailing '/'
                    Comment = "",
...
//        private const long DefaultBufferThreshold = 4L * 1024 * 1024; // 64 MB
